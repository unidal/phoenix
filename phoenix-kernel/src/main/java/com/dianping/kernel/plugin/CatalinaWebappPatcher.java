...
public class CatalinaWebappPatcher implements WebappPatcher {
	...
	public void applyKernelWebXml() throws Exception {
		...
		Digester digester = DigesterFactory.newDigester(ctx.getXmlNamespaceAware(), ctx.getXmlValidation(),
				new WebRuleSet());
	...
	public void mergeWebResources() throws Exception {
		...
		DirContext context = (DirContext) Proxy.newProxyInstance(DirContext.class.getClassLoader(),
				new Class[]{DirContext.class}, new CompositeDirContextProxy(dirContext, kernelFileDirContext));

		m_loader.setFieldValue(proxyDirContext, "dirContext", context);
	}
	...
	private void filterWebFilter() {
		...
		FilterMap[] filterMaps = m_context.findFilterMaps();
		StandardContext kernelContext = (StandardContext) m_context.getServletContext().getAttribute(
	...
	protected void sortListener() {
		...
		for (String dlisten : discardListenerList) {
			m_loader.getLog().warn(
					"----No match is found for the listener::" + dlisten + " and rule::"
							+ m_context.findParameter(dlisten));
