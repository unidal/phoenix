...
public class DefaultStatusReporter implements StatusReporter {
	...
	private MessageFormat m_format = new MessageFormat("[{0,date,yyyy-MM-dd HH:mm:ss}] [{1}] [{2}] {3}");
	...
	public synchronized void log(String message) {
		log(DEFAULT_LOG, DEFAULT_LOG, message);
	}

	public synchronized void log(String category, String subCategory, String message) {
	...
	private void storeMessage(String category, String subCategory, String message) {
		if (category != null && !category.equals(DEFAULT_LOG) && subCategory != null && !subCategory.equals(DEFAULT_LOG)) {
