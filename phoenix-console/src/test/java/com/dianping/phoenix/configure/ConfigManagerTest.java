...
public class ConfigManagerTest {
	...
	public void test() throws Exception {
		...
		Assert.assertEquals("ssh://git@10.1.4.81:58422/phoenix-kernel.git", manager.getGitOriginUrl("phoenix-kernel"));
		Assert.assertEquals("target/gitrepo/phoenix-kernel", manager.getGitWorkingDir("phoenix-kernel"));

		Assert.assertEquals("http://localhost:3473/phoenix/agent/deploy?op=deploy&deployId=1&domain=user-web&"
		      + "version=0.1-SNAPSHOT&warType=phoenix-kernel&gitUrl=ssh://git@10.1.4.81:58422/phoenix-kernel.git&"
		      + "qaServiceUrlPrefix=http://192.168.26.23:8080/qa/service/task&qaServiceTimeout=300000",
		      manager.getDeployUrl("phoenix-kernel", "localhost", 1, "user-web", "0.1-SNAPSHOT", false));
		Assert.assertEquals("http://localhost:3473/phoenix/agent/deploy?op=deploy&deployId=1&domain=user-web&"
		      + "version=0.1-SNAPSHOT&warType=phoenix-kernel&gitUrl=ssh://git@10.1.4.81:58422/phoenix-kernel.git",
		      manager.getDeployUrl("phoenix-kernel", "localhost", 1, "user-web", "0.1-SNAPSHOT", true));
